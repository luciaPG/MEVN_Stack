<template>
  <div class="editar-serie-view">
    <div class="header">
      <h1>Editar Serie</h1>
      <router-link to="/" class="back-link">← Volver al listado</router-link>
    </div>
...
// Cargar los datos de la serie al entrar
onMounted(async () => {
  try {
    const response = await axios.get(
      `http://localhost:5000/api/series/${route.params.id}`
    );
    serie.value = response.data;
  } catch (error) {
    console.error("Error al cargar la serie:", error);
    alert("No se pudo cargar la serie. Por favor intenta nuevamente.");
    router.push("/");
  }
});
...
const handleSubmit = async () => {
  try {
    ...
    await axios.put(`http://localhost:5000/api/series/${route.params.id}`, {
      nombre: serie.value.nombre.trim(),
      genero: serie.value.genero.trim(),
      sinopsis: serie.value.sinopsis.trim(),
    });

    router.push(`/detalles/${route.params.id}`);
  } catch (error) {
    console.error("Error al actualizar la serie:", error);
    alert(
      "Ocurrió un error al actualizar la serie. Por favor intenta nuevamente."
    );
  } finally {

<template>
  <div class="editar-temporada-view">
    <div class="header">
...
const serieId = ref("");
const temporadaId = ref(route.params.id);
...
onMounted(async () => {
  try {
    ...
    temporada.value.numeroTemporada = response.data.numeroTemporada; // Corregido aquí
    temporada.value.serie = serieId.value;
  } catch (error) {
    console.error("Error al cargar la temporada:", error);
    errorMessage.value =
      "No se pudo cargar la temporada. Por favor intenta nuevamente.";
  }
});
...
const handleSubmit = async () => {
  try {
    ...
    if (
      ...
      errorMessage.value = "El número de temporada debe ser mayor a 0";
    ...
    if (error.response?.data?.message) {
      ...
      errorMessage.value = "Ocurrió un error al actualizar la temporada";
    }
  }
};
